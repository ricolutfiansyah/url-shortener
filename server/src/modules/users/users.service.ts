...
export const userService = {
    ...
    async userTokenRefresh(incomingRefreshToken: string) {
        ...
        const newAccessToken = await generateAccessToken(session.userId, session.user.role)

        return { user: session.user, accessToken: newAccessToken }
    },

    async userLogout(incomingRefreshToken: string) {
        await db.delete(sessions).where(eq(sessions.refreshToken, incomingRefreshToken))
